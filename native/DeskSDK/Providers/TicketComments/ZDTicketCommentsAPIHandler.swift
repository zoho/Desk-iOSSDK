...
@objc open class ZDTicketCommentsAPIHandler : NSObject{
    ...
    ///   - optionalParams: Optional paramerters can be given by
    ///   - onComplition: Oncomplition data will ticket object or Error message and status code of the reponce.
    @objc open static func createTicketComment(_ orgID:String = String().getZDOrgId(),ticketID:String,isPublic:Bool,content:String,optionalParams:Parameters = Parameters(),onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.TicketsComments.createTicketComment, ticketID), method: .POST, paramType: .json, parameters: optionalParams, headers: ["orgId":orgID])
    ...
    @objc open static func updateTicketComment(_ orgID:String = String().getZDOrgId(),ticketID:String,commendId:String,content:String,optionalParams:Parameters = Parameters(),onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.TicketsComments.updateTicketComment, ticketID,commendId), method: .PATCH, paramType: .json,parameters:optionalParams, headers: ["orgId":orgID])
        request.parameters = ["content":content]
    ...
    @objc open static func deleteTicketComment(_ orgID:String = String().getZDOrgId(),ticketID:String,commendId:String,onComplition:@escaping ((Error?,Int)->())) -> Void{
    ...
    @objc open static func getTicketComment(_ orgID:String = String().getZDOrgId(),ticketID:String,commendId:String,onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
    ...
    @objc public static func getAllTicketComments(_ orgID:String = String().getZDOrgId(),ticketID:String,optionalParams:Parameters = Parameters(), onComplition:@escaping (([ZDTicketComment]?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.TicketsComments.getAllTicketComment, ticketID),parameters:optionalParams, headers:["orgId":orgID])
