...
@objc public class ZDTicketCommentsAPIHandler : NSObject{
    ...
    @objc public static func createTicketComment(_ orgID:String = String().getOrgId(),ticketID:String,isPublic:Bool,content:String,addtionalParams:Parameters = Parameters(),onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.createTicketComment, ticketID), method: .POST, paramType: .json, parameters: addtionalParams, headers: ["orgId":orgID])
    ...
    @objc public static func updateTicketComment(_ orgID:String = String().getOrgId(),ticketID:String,commendId:String,content:String,addtionalParams:Parameters = Parameters(),onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.updateTicketComment, ticketID,commendId), method: .PATCH, paramType: .json,parameters:addtionalParams, headers: ["orgId":orgID])
        request.parameters = ["content":content]
    ...
    @objc public static func deleteTicketComment(_ orgID:String = String().getOrgId(),ticketID:String,commendId:String,onComplition:@escaping ((Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path:  String(format: URLPathConstants.deleteTicketComment, ticketID,commendId), method: .DELETE, paramType: .path, headers: ["orgId":orgID])
        ZDBaseRequester.getJsonDicReponce(baseRequest: request) { (json, data, error, statusCode) in
            onComplition(error, statusCode)
        }
    }
    
    @objc public static func getTicketComment(_ orgID:String = String().getOrgId(),ticketID:String,commendId:String,onComplition:@escaping ((ZDTicketComment?,Error?,Int)->())) -> Void{
        let request = ZDBaseRequest(path: String(format: URLPathConstants.getTicketComment, ticketID,commendId),headers:["orgId":orgID])
    ...
    @objc public static func getAllTicketComments(_ orgID:String = String().getOrgId(),ticketID:String,from:Int = 0,limit:Int = 50,additionalParams:Parameters = Parameters(), onComplition:@escaping (([ZDTicketComment]?,Error?,Int)->())) -> Void{
        
        let request = ZDBaseRequest(path: String(format: URLPathConstants.getAllTicketComment, ticketID),parameters:additionalParams, headers:["orgId":orgID])
